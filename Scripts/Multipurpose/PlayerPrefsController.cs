...
public class PlayerPrefsController : MonoBehaviour
{
    ...
    public static void SetDifficultyValue(float inDifficulty)
    {
        if (inDifficulty <= MAX_DIFFICULTY &&
           inDifficulty >= MIN_DIFFICULTY)
        {
            PlayerPrefs.SetInt(DIFFICULTY_KEY, (int)inDifficulty);
        }
        else
        {
            Debug.LogError("The chosen difficulty is out of range.");
        }
    }

    public static int GetDifficultyValue(int inDefault = 1)
    {
        return PlayerPrefs.GetInt(DIFFICULTY_KEY, inDefault);
    }
}
